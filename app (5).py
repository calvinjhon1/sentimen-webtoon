...
with st.sidebar:
    ...
    st.markdown("✅ Random Forest + Uni+Bi+Trigram")
    st.markdown("**Akurasi:** 90.98%")
    st.markdown("**F1-Score:** 90.77%")
    st.markdown("---")
...
if menu == "🏠 Beranda":
    ...
    with right:
        ...
        st.success("**Random Forest + Unigram+Bigram+Trigram (1,3)**")
        for s in [
            "1️⃣ Case Folding",
    ...
    ca.success("**Temuan 1**\n\nUnigram+Bigram+Trigram (1,3) terbaik pada Random Forest — menangkap konteks frasa lebih kaya dengan class_weight balanced.")
    cb.info("**Temuan 2**\n\nRandom Forest unggul keseluruhan (F1 90,77%) setelah penanganan class imbalance. SVM kompetitif di konfigurasi Unigram+Bigram.")
        ...
        if not user_input.strip():
            ...
            if not clean.strip():
                ...
                with col_detail:
                    ...
                    st.info("**Model:** Random Forest + Unigram+Bigram+Trigram (1,3)\n\n**Accuracy:** 90.98% | **F1-Score:** 90.77%")
